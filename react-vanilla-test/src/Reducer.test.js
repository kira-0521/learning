...
describe('Reducer of ReduxToolkit', () => {
  ...
  describe('incrementByAmountAction', () => {
    let initialState = {
      mode: 0,
      value: 1,
    }

    it('Should increment by value with mode 0', () => {
      const action = { type: incrementByAmount.type, payload: 3 }
      const state = reducer(initialState, action)
      expect(state.value).toEqual(4)
    })
    it('Should increment by value with mode 1', () => {
      initialState = {
        mode: 1,
        value: 1,
      }
    ...
    it('Should increment by value with mode 2', () => {
      initialState = {
        mode: 2,
        value: 1,
      }
